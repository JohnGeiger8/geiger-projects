...
class SnakeWindow(QMainWindow):
    ...
    def initGame(self):
        ...
        self.setGeometry(300, 50,
                         self.gameFrame.gameWidth * self.gameFrame.squareSize,
...
class SnakeFrame(QFrame):
    ...
    gameHeight = 40
    gameWidth = 40
    isDirectionChanging = False  # Prevents 2 quick moves in a row
    ...
    def initFrame(self):
        ...
        self.initialSnakePosition = [(i, yPosition) for i in range(xPosition-3,
                                     xPosition+1)]
    ...
    def moveSnake(self):
        ...
        if snakeHead == self.foodPosition:
            ...
            boardSize = self.gameWidth * self.gameHeight - 1
            # Check if user filled the entire screen (and thus won)
            if len(self.snake.bodyPositions) == boardSize:
                self.gameTimer.stop()
        ...
        # Check that snake isn't running into the top or bottom
        elif snakeHead[1] not in range(0, self.gameHeight):
            self.gameTimer.stop()
            self.gameState = GameState.GameOver
            self.update()

        # Check that snake isn't running into the wall
        elif snakeHead[0] not in range(0, self.gameWidth):
            self.gameTimer.stop()
            self.gameState = GameState.GameOver
            self.update()
    ...
    def keyPressEvent(self, event):
        ...
        if self.isDirectionChanging is False:
            if key == Qt.Key_Left and direction != Direction.Right:
                self.snake.direction = Direction.Left
            elif key == Qt.Key_Right and direction != Direction.Left:
                self.snake.direction = Direction.Right
            elif key == Qt.Key_Up and direction != Direction.Down:
                self.snake.direction = Direction.Up
            elif key == Qt.Key_Down and direction != Direction.Up:
                self.snake.direction = Direction.Down

            self.isDirectionChanging = True

        # Start/Restart game
        elif key == Qt.Key_Space and self.gameState != GameState.Running:
            self.gameState = GameState.Running
            self.initFrame()
            self.startGame()
    ...
    def drawTitleMessages(self, title, subTitle, painter):
        ...
        painter.drawText(0, self.squareSize * self.gameHeight // 3 - 40,
                         self.squareSize * self.gameWidth, 40, Qt.AlignCenter,
                         title)

        font.setPointSize(18)
        painter.setFont(font)
        painter.drawText(0, self.squareSize * self.gameHeight // 3,
                         self.squareSize * self.gameWidth, 30, Qt.AlignCenter,
                         subTitle)
